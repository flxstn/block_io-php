...
if(!class_exists("BlockIo")) {
    class BlockIo {
        ...
        public function __construct( $api_key, $pin, $api_version = 2 ) { // the constructor
            $this->api_key = $api_key;
            $this->pin     = $pin;
            $this->version = $api_version;

            $this->withdrawal_methods = array(
                "withdraw",
                "withdraw_from_user",
                "withdraw_from_users",
                "withdraw_from_label",
        ...
        private function _withdraw( $name, $args = array() ) { // withdraw method to be called by __call
            ...
            if ( $response->status == 'success' && array_key_exists( 'reference_id', $response->data ) ) { // we have signatures to append
                ...
                if ( $key->getPublicKey() != $response->data->encrypted_passphrase->signer_public_key ) {
        ...
        private function _sweep( $name, $args = array() ) { // sweep method to be called by __call
        ...
        public function initKey() { // grants a new Key object
            return new BlockKey();
        }

        private function pbkdf2( $password, $key_length, $salt = "", $rounds = 1024, $a = 'sha256' ) { // PBKDF2 function adaptation for Block.io
        ...
        public function pinToAesKey( $pin ) { // converts the given Secret PIN to an Encryption Key
    ...
    class BlockKey {
        ...
        public function deterministicGenerateK( $message, $key ) { // key in hex, message as it is
            // RFC6979
        ...
        public function getWif( $network = "BTC" ) {
            ...
            if ( $this->c ) {
                $secretKey .= '01';
            } // set the compression flag if we need it
        ...
        public function fromPassphrase( $pp ) {  // take a sha256 hash of the passphrase, and then set it as the private key
        ...
        public function fromWif( $pp ) { // extract the private key from the key in Wallet Import Format

            // TODO validation

            if ( $this->validateWifKey( $pp ) === false ) {
